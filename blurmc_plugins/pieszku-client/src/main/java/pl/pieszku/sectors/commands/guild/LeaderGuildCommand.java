...
public class LeaderGuildCommand extends GuildSubCommand{


    private final GuildService guildService = BukkitMain.getInstance().getGuildService();

    public LeaderGuildCommand() {
        super("zastepca", "", "/g zastepca <nick>", "");
    }
    ...
    public boolean onCommand(Player player, String[] args) {
        ...
        this.guildService.findGuildByMember(player.getName()).ifPresent(guild -> {
            ...
            if(guild.getLeader().equalsIgnoreCase(nickName)){
                player.sendMessage(ChatUtilities.colored("&4Błąd: &cPodany gracz jest już zastępcą!"));
                return;
            }
            ...
            guild.sendMessage("&3&lGILDIA &8>> &fNowy zastępcą gildii: &b" + nickName, BukkitMain.getInstance().getSectorService());
        });
        return false;
    }
